...
idt CreateDateFromAge( wxXmlNode* node, idt baseID, idt refID, int* pseq )
{
    ...
    xmlCreateLink( node, "^"+recDate::GetIdStr( dateID ) );
...
idt CreatePlace( wxXmlNode* node, idt refID, int* pseq )
{
    ...
    xmlCreateLink( node, "^"+recPlace::GetIdStr( placeID ) );
    ...
    wxXmlNode* row, idt refID, idt eventID, idt dateID, const wxString& address, int* pseq )
{
    ...
    while( row ) {
        ...
        if( indID && !name.IsEmpty() ) {
            idt perID = AddPersona( refID, indID, name, pseq );
...
//            }
            ...
            if( samecountyStr.Mid( 0, 1 ) == "y" ) {
                ...
                samecounty = true;
                xmlCreateLink( data, "^"+recPlace::GetIdStr( bplaceID ) );
            } else {
                samecounty = false;
            }
            if( ageID || samecounty ) {
...
void ProcessCensusIndividuals( 
    wxXmlNode* row, idt refID, idt eventID, idt dateID, int* pseq )
{
    wxXmlNode* data;
    wxXmlNode* condNode;
    wxXmlNode* span;
    idt indID, bplaceID, ageID, rsID, attID;
    int attSeq = 0;
    wxString name;
    idt headPerId = 0;
    wxString relStr;
    ...
    while( row ) {
        ...
        indID = GetIndividualAnchor( data, &name );
        if( indID && !name.IsEmpty() ) {
            idt perID = CreateRefPersona( SEX_Unstated, refID, indID, name, pseq );
            // Add persona to Census Event
            ep.f_id = 0;
            ep.f_per_id = perID;
            ep.f_per_seq = ++personaSeq;
            ...
            if( headPerId == 0 || relStr == "Head" ) {
                ...
                xmlCreateLink( data, "^"+recRelationship::GetIdStr( rsID ) );
            }
            ...
            if( attID ) {
                xmlCreateLink( condNode, "^"+recEvent::GetIdStr( attID ) );
            }
            ...
            if( data ) {
                ...
                if( attID ) {
                    xmlCreateLink( span, "^"+recEvent::GetIdStr( attID ) );
                }
            }
        }
        row = xmlGetNext( row, "tr" );
...
void CreateUkCensus( idt refID, idt dateID, wxXmlNode* refNode, const wxString& title )
{
    ...
    if( dateID == g_1841CensusDateID ) {
        ...
        ProcessCensusIndividuals( row, refID, eventID, dateID, &refSeq );
    }
}
...
void CreateIgiBaptism( idt refID, wxXmlNode* refNode )
{
    ...
    idt perID = CreateRefPersona( sex, refID, indID, name, &refSeq );
    ...
    idt fatherPerID = 0;
    if( !name.IsEmpty() ) {
        fatherPerID = CreateRefPersona( SEX_Male, refID, fatherIndID, name, &refSeq );
    }
    ...
    if( !birthStr.IsEmpty() ) {
        ...
        xmlCreateLink( birthDateCell, "^"+recDate::GetIdStr( dateID ) );
        xmlCreateLink( birthEventCell, 0, 5, "^"+recEvent::GetIdStr( eventID ) );
    }
    if( !chrisStr.IsEmpty() ) {
        ...
        wxXmlNode* link = xmlCreateLink( chrisCell, 0, pos, "^"+recDate::GetIdStr( dateID ) );
        xmlCreateLink( link->GetNext(), 2, -1, "^"+recPlace::GetIdStr( placeID ) );
        xmlCreateLink( chrisEventCell, 0, 11, "^"+recEvent::GetIdStr( eventID ) );
    }
    if( !deathStr.IsEmpty() ) {
        dateID = CreateDate( deathStr, refID, &refSeq );
        placeID = 0;
        eventID = CreateDeathEvent( refID, perID, dateID, placeID, &refSeq );
        xmlCreateLink( deathCell, "^"+recDate::GetIdStr( dateID ) );
        xmlCreateLink( deathEventCell, 0, 5, "^"+recEvent::GetIdStr( eventID ) );
    }
    if( !burialStr.IsEmpty() ) {
        dateID = CreateDate( burialStr, refID, &refSeq );
        placeID = 0;
        eventID = CreateBurialEvent( refID, perID, dateID, placeID, &refSeq );
        xmlCreateLink( burialCell, "^"+recDate::GetIdStr( dateID ) );
        xmlCreateLink( burialEventCell, 0, 6, "^"+recEvent::GetIdStr( eventID ) );
    }
