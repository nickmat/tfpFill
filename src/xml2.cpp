...
bool wxXmlNode::IsWhitespaceOnly_() const
{
...
bool wxXmlDocument::Save(const wxString& filename, int indentstep, int flags) const
{
    wxFileOutputStream stream(filename);
    if (!stream.IsOk())
        return false;
    return Save(stream, indentstep, flags);
}
...
static void EndDoctypeHnd(void *userData)
{
    wxXmlParsingContext *ctx = (wxXmlParsingContext*)userData;
}
                ...
                int indentstep)
{
    ...
    switch (node->GetType())
    {
        ...
        case wxXML_ELEMENT_NODE:
            ...
            {
                if( node->GetName() == "td" ) {
                    rc = OutputString(stream, wxS("></td>"), convMem, convFile);
                } else {
                    rc = OutputString(stream, wxS("/>"), convMem, convFile);
                }
            }
...
bool wxXmlDocument::Save(wxOutputStream& stream, int indentstep, int flags) const
{
    if ( !IsOk() )
        return false;
    ...
    bool rc = true;
    if ( flags & wxXMLDOC_SAVE_AUTO_DECLARATION )
    {
        wxString dec = wxString::Format(
                                        wxS("<?xml version=\"%s\" encoding=\"%s\"?>\n"),
                                        GetVersion(), GetFileEncoding()
                                       );
        rc = OutputString(stream, dec, convMem.get(), convFile.get());
    }
